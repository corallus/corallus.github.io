...
const Section = (props) => (
  <section
    id={props.id}
    className="resume-section p-3 p-lg-5 d-flex d-print-block flex-column justify-content-center"
  >
    {props.children}
  </section>
)
      ...
      <Section id="about">
        <header>
          <h1>
            ...
            <span className="text-primary">{about.frontmatter.surname}</span>
          </h1>
          <div className="subheading">{about.frontmatter.title}</div>
        </header>
        <div
          className="my-4 lead"
        ...
        <section title="Skills" id="skills">
          ...
          <Row>
            {about.frontmatter.skills &&
              about.frontmatter.skills.map((skill, i) => (
                    ...
                    <Card.Body className="">
                      {skill.skills &&
                        skill.skills.map((subskill, j) => (
                          <Card.Text key={j}>
                            <Badge variant="secondary" className="mr-2 my-2">
                              {subskill.name}
                            </Badge>
                            {subskill.skills &&
                              subskill.skills.map((subsubskill, k) => (
                                <Badge
                                  variant="primary"
                                  className="mx-2 my-2"
                                  key={k}
        ...
        <Nav className="social-icons">
          <Nav.Link
            href={about.frontmatter.linkedin}
            target="_blank"
            className="pl-0"
          >
            <FaLinkedin size="64" />
